...
/** Labels and values from cleanProperties2 */
export interface CleanedProperties {
 ...
 * @private
 */
const keysFromObject = (obj: Record<string, unknown> | null | undefined): string[] => {
  if (!obj) {
    return [];
  }
 ...
 */
const setAddAll = <T>(iteratable: Iterable<T>, targetSet: Set<T>): Set<T> => {
  for (const value of iteratable) targetSet.add(value);
  return targetSet;
};
  ...
  if (!propertyOrFnMap) {
    ...
    for (let i = 0; i < propertyOrFnMap.length; i += 1) {
      ...
      if (typeof propertyOrFnKey === 'string') {
        ...
        propertyEntries.push(propertyOrFnKey as [string, PropertyOrFn | string]);
      }
    }
  } else if (propertyOrFnMap instanceof Map) {
    propertyEntries = [...(propertyOrFnMap as Map<string, PropertyOrFn | string>).entries()];
  } else {
